...
// ~ 보다 큼 또는 작음 → boolean
let lessThan = number[0] < number[3]; // true
let greaterThan = number[0] < number[1]; // true

// ~ 보다 크거나 작음 또는 같음 → boolean
let lessThanAndEqual = number[4] <= number[1]; // true
let greaterThanAndEqual = number[5] >= number[0]; // true
...
// 동일(똑같다) → boolean
let same = number[3] === number[4]; // false
let notSame = number[3] !== number[4]; // true
